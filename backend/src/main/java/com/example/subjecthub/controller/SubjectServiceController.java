...
public class SubjectServiceController implements SubjectServiceApi {
    ...
    @Override
    public List<Subject> getSubjects(String subjectCode, String name, Long facultyId, String facultyName,
        Double ratingStart, Double ratingEnd, Integer creditPoints, String instructor) {

        // TODO: Move subjectCode retrieval to it's own method.
        // It is unique within a uni. We shouldn't return a list.
        Application.log.info("Received:\nsubjectCode {}\nfacultyId: {}\nratingStart: {}\n ratingEnd: {}", subjectCode,
            facultyId, ratingStart, ratingEnd);
        ...
        if (facultyName != null) {
            subjectStream = subjectStream.filter(s -> s.getFaculty().getName().contains(facultyName));
        }
        ...
        if (ratingStart != null || ratingEnd != null) {
            ...
            subjectStream = subjectStream.filter(s -> s.getRating() >= start).filter(s -> s.getRating() <= end);
        }
