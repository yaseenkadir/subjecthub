...
public class SubjectServiceController implements SubjectServiceApi {
  ...
  @Override
  public List<Subject> getSubjects(String subjectCode, String name,
                                   Long facultyId, String facultyName,
                                   Double ratingStart, Double ratingEnd,
                                   Integer creditPoints, String instructor) {
    ...
    if (facultyId != null) {
      subjectStream =
          subjectStream.filter(s -> s.getFaculty().getId().equals(facultyId));
    }

    if (facultyName != null) {
      subjectStream = subjectStream.filter(
          s -> s.getFaculty().getName().contains(facultyName));
    }

    if (ratingStart != null || ratingEnd != null) {
      final Double start =
          (ratingStart == null ? new Double("0.0") : ratingStart);
      final Double end = (ratingEnd == null ? new Double("10.0") : ratingEnd);

      subjectStream = subjectStream.filter(s -> s.getRating() >= start)
